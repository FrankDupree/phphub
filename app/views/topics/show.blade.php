...
<div class="col-md-9 topics-show main-col">
    ...
    <div class="infos panel-heading">
      ...
      <div class="pull-right avatar_large">
        <a href="{{ route('users.show', $topic->user->id) }}">
             @if ($topic->user->image_url)
    <img class="img-thumbnail avatar" src="{{profileImagePath() .$topic->user->name.'/' .$topic->user->image_url }}" style="width:65px; height:65px;">
    @else
        <img class="img-thumbnail avatar" src="{{profileImagePath() .'blank.jpg'}}" style="width:65px; height:65px;">  
    @endif
        </a>
      </div>
  ...
  <div class="replies panel panel-default list-panel replies-index">
    <div class="panel-heading">
      <div class="total">{{ lang('Total Replies') }}: <b>{{ $replies->getTotal() }}</b> </div>
    </div>
    ...
    {{ Form::open(['route' => 'replies.store', 'id' => 'reply-form', 'method' => 'post']) }}
      <input type="hidden" name="topic_id" value="{{ $topic->id }}" />
        ...
        <div class="form-group">
            @if ($currentUser)
              {{ Form::textarea('body', null, ['class' => 'form-control',
                                                ...
                                                'id' => 'reply_content']) }}
            @else
              {{ Form::textarea('body', null, ['class' => 'form-control', 'disabled' => 'disabled', 'rows' => 5, 'placeholder' => lang('Log in or register to comment.')]) }}
            @endif
        </div>
        ...
        <div class="form-group status-post-submit">
            ...
            <span class="help-inline" title="Or Command + Enter"></span>
        </div>
