    ...
    @foreach ($topics as $topic)
     <li class="list-group-item media {{ !$column ?:'col-sm-6'; }}" style="margin-top: 0px;">
        <a class="pull-right" href="{{ route('topics.show', [$topic->id]) }}" >
            <span class="badge badge-reply-count" id="{{$topic->node->name}}"> {{ $topic->reply_count }} </span>
        </a>
        <div class="avatar pull-left">
            <a href="{{ route('users.show', [$topic->user_id]) }}">
    @if ($topic->user->image_url)
    <img class="media-object img-thumbnail avatar" src="{{profileImagePath() .$topic->user->name.'/' .$topic->user->image_url }}" alt="{{{ $topic->user->name }}}" style="width:48px;height:48px;">
    @else
        <img class="media-object img-thumbnail avatar" src="{{profileImagePath() .'blank.jpg'}}" alt="{{{ $topic->user->name }}}" style="width:48px;height:48px;">  
    @endif
            </a>
        </div>
...
@else
   <div class="empty-block">{{ lang('There are no feeds Yet') }}</div>
@endif
